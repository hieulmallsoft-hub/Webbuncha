package vn.hoidanit.springrestwithai.features.orders.application;
...
import vn.hoidanit.springrestwithai.features.orders.infrastructure.persistence.OrderSpecifications;
...
import vn.hoidanit.springrestwithai.features.notifications.push.application.PushNotificationService;
...
public class OrderService {
    ...
    public Order getOrderById(Long id, String userEmail, boolean isAdmin) {
        return getAccessibleOrder(id, userEmail, isAdmin);
    }

    public Order getAccessibleOrder(Long id, String userEmail, boolean isAdmin) {
    ...
    public Order createOrder(String userEmail, OrderRequest request) {
        ...
        Order saved = this.orderRepository.saveAndFlush(order);
        Order createdOrder = getOrderByIdInternal(saved.getId());
        try {
            notificationService.createOrderNotification(createdOrder);
        } catch (Exception ex) {
            log.warn("Failed to create order notification for order {}: {}", createdOrder.getId(), ex.getMessage(),
                    ex);
        }

        try {
            pushNotificationService.sendOrderCreatedNotification(createdOrder);
        } catch (Exception e) {
    ...
    public Order updatePaymentResult(Long id, boolean success) {
        Order existing = getOrderByIdInternal(id);
        if (existing.getStatus() == Order.OrderStatus.COMPLETED) {
            return existing;
        }
        if (existing.getStatus() == Order.OrderStatus.PAID) {
            return existing;
        }
        ...
        if (success) {
            existing.setStatus(Order.OrderStatus.PAID);
        } else if (existing.getStatus() == Order.OrderStatus.PENDING
                || existing.getStatus() == Order.OrderStatus.CONFIRMED) {
